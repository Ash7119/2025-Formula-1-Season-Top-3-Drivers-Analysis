...
# 👉 Cache setup - works on both local and Streamlit Cloud
cache_dir = '/tmp/f1_cache' if os.path.exists('/tmp') else '.fastf1_cache'
...
# 👉 UPDATED: Added telemetry=False, weather=False, messages=False to reduce API calls
...
def load_race_session(year, race_name_or_round, session_type='R'):
    try:
        ...
        # 👉 Only load telemetry for qualifying and race (not season overview)
        session.load()
        return session
    except Exception as e:
...
# 👉 NEW: Separate lightweight session loader for season overview (no telemetry)
@st.cache_data(ttl=3600)
def load_session_results_only(year, round_num, session_type='R'):
    try:
        session = fastf1.get_session(year, round_num, session_type)
        session.load(telemetry=False, weather=False, messages=False)  # 👈 Only results, no telemetry
        return session
    except Exception as e:
        return None
...
def load_all_season_data(year, schedule):
    ...
    for idx, race in completed_races.iterrows():
        ...
        try:
            # 👉 Use lightweight loader (no telemetry)
            race_session = load_session_results_only(year, round_num, 'R')
            if race_session is None:
                continue
            
            time.sleep(0.5)  # 👈 Delay to avoid rate limiting
            
            quali_session = load_session_results_only(year, round_num, 'Q')
            ...
            try:
                sprint_session = load_session_results_only(year, round_num, 'S')
                ...
                time.sleep(0.5)  # 👈 Delay after sprint too
...
def degradation_chart(laps_df, session):
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=True, gridwidth=1, gridcolor='#333333'),
        yaxis=dict(showgrid=True, gridwidth=1, gridcolor='#333333')
    )
        ...
        for compound in compounds:
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    ...
                    line=dict(color=driver_config['color'], width=2)
                ),
...
with tab3:
    ...
    - Delta Time Chart: Comparing the drivers' fastest lap times in qualifying sessions.
    - Minisector Plot: A plot of the circuit showing which driver was fastest in each sector.
    """)
    ...
    with st.expander("Delta Time Comparison", expanded=True):
        ...
        if delta_chart is not None:
            ...
            st.warning("Could not create delta time chart.")  
    
    with st.expander("Mini-Sector Performance Comparison", expanded=True):
        st.markdown("""
        This plot shows which driver was fastest in each mini-sector of the circuit.
        """)
        ...
        if minisector_chart is not None:
            ...
            st.warning("Could not create minisector comparison chart.")
...
with tab4:
    ...
    st.markdown("""
    This section provides insights into the drivers' race performance, including tyre strategy,
    degradation, lap times, and race pace comparison.
    """)
    ...
    with st.expander("Tyre Strategy Analysis", expanded=True):  
        ...
        Colors: Red=Soft, Yellow=Medium, White=Hard, Green=Intermediate, Blue=Wet.
        """)
    ...
    with st.expander("Tyre Degradation Analysis", expanded=True):
        st.markdown("""
        The line plot shows tyre degradation compared to the first lap of each stint.
        """)
    ...
    with st.expander("Lap Time Scatter Plot (by Tire Compound)", expanded=True):
        ...
        st.markdown("""
        Dot fill = Tire compound. Dot outline = Driver color.
        """)
